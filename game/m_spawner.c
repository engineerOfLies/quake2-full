...
void spawner_pain(edict_t* self, edict_t* other, float kick, int damage)
{
	gi.cprintf(other, PRINT_HIGH, "ow i've been hit");
...
void spawner_die(edict_t* self, edict_t* inflictor, edict_t* attacker, int damage, vec3_t point)
{
	int		n;

	if (self->health <= self->gib_health)
	{
		gi.cprintf(attacker, PRINT_HIGH, "spawner has been killed");
		self->deadflag = DEAD_DEAD;
		return;
	}
...
void SP_monster_spawner (edict_t *self)
{
	...
	self->pain = spawner_pain;
	self->die = spawner_die;
